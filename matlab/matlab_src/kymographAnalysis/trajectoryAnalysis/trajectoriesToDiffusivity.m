...
if version == 2
    ...
    if length(position) > 3
        ...
        displacement = diff(position); %position(2:end) - position(1:end-1);
        Dt = diff(timeFrame); 
        relevant = Dt == 1;
        ...
        if fun_flow==0
            ...
            velocity = mean(displacement(relevant));
        end
    
        diffusivity0 = mean((displacement(relevant) - velocity).^2)/2;
    
        correction = (displacement(1:end-1)-velocity).*(displacement(2:end)-velocity);
        correction = mean(correction(relevant(1:end-1) & relevant(2:end)));

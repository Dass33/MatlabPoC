...
def get_aggregated_results(available_results):
    ...
    combined_df = pd.concat(all_detections, ignore_index=True) if all_detections else pd.DataFrame()
    tracks_summary_df = pd.DataFrame(all_tracks_summary) if all_tracks_summary else pd.DataFrame()

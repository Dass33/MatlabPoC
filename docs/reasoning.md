...
3.  **Old tabs reconnect but never rerun the script** ("zombie" sessions): the server creates a session, no script runs, nothing new is ever delivered - so a fix shipped in the app never reaches the tabs that need it, and they hold the websocket forever. During the migration a server-side kicker force-reran such sessions to deliver the watchdog; it was removed once no pre-watchdog tabs remained (commit f6a133e has it), because current tabs keep their watchdog across reconnects and the probe-missing fallback (below) covers fresh containers.
...
5.  **A missing probe file is a safe state.** The watchdog treats repeated probe-fetch failures as "no job" and disconnects after a few retries - correct on a fresh container that never ran a script, and the reason the probe thread can start lazily with the first script run.

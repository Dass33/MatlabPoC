...
def main():
    ...
    try:
        subprocess.run(
            [sys.executable, str(scripts_dir / "compile_matlab.py")], check=True
        )
    ...
    try:
        subprocess.run(
            [sys.executable, str(scripts_dir / "build_streamlit.py")], check=True
        )
    except subprocess.CalledProcessError as e:
    ...
    try:
        ...
        print(
            "Error: 'docker' command not found. Make sure Docker is installed.",
            file=sys.stderr,
        )
        sys.exit(1)
    except subprocess.CalledProcessError as e:

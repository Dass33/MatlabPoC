...
def main():
    ...
    if git_dir.exists():
        try:
            ...
            print("If you cloned this repository without git, please ensure the submodules are present.")
    else:
        print("Not a git repository or .git folder not found. Skipping git submodule update.")
    ...
    if not env_file.exists():
        if env_example.exists():
            ...
            print(f"Warning: {env_example.name} not found. Creating a blank {env_file.name} file.")
            env_file.touch()
    else:

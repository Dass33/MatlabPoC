function HR = diffusivityToSize (Deff, channel_area)
...
if isnan(Deff) == 0
    ...
    %for i=1:length(Deff)
        HR0 = kB*T./(6*pi*ny*Deff*1e-6) *1e12;
        f0 = hindranceFactor(HR0./sqrt(channel_area/pi)) * kB*T./(6*pi*ny*Deff*1e-6) *1e12 - HR0;
        HR1 = HR0*0.9;
        f1 = hindranceFactor(HR1./sqrt(channel_area/pi)) * kB*T./(6*pi*ny*Deff*1e-6) *1e12 - HR1;
        HR2 = HR0;
        
    %     close all
    %     plot(HR0, f0,'.'); hold on
    %     plot(HR1, f1,'.'); hold on
        while abs(HR0./HR1 - 1) > 0.0001
            HR2 = real(HR1 - (HR1-HR0)/(f1-f0)*f1);
            
            if imag(HR2)~=0
                disp('')
            end
            f2 = real(hindranceFactor(HR2./sqrt(channel_area/pi)) * kB*T./(6*pi*ny*Deff*1e-6) *1e12 - HR2);
            HR0=HR1;
            HR1=HR2;
            f0=f1;
            f1=f2;

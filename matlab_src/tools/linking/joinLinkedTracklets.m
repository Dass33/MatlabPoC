function Tracks = joinLinkedTracklets(matchedTrackletIds, unmatchedRows, Tracklets)
    ...
    for iMatchedPair = 1:size(matchedTrackletIds,1)
        ...
        % otherwise, create new track
            ...
            Tracks.nTracks = Tracks.nTracks + 1;
            Tracks.tracklet_ids{Tracks.nTracks} = [sourceTracklet; destinationTracklet];
    ...
    Tracks.nTracks = Tracks.nTracks + length(singleTrackletTracks);
    
    % check total number of tracklets
    if ~sum(cellfun(@length, Tracks.tracklet_ids)) == Tracklets.nTracks

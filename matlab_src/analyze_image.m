function [Detections, Y, C, FinalTracks] = analyze_image(R, options)
    % ANALYZE_IMAGE Performs the tracking analysis pipeline on a raw image.
    %
    % Inputs:
    %   R: Raw image matrix (Double).
    %   options: Name-Value pairs for pipeline parameters.
    %
    % Outputs:
    %   Detections: Struct containing detected spots and their properties (refined).
    %   Y: Denoised image matrix.
    %   C: Contrast image matrix.
    %   FinalTracks: Struct containing the final linked tracks.
    ...
    arguments
        ...
        % --- Contrast Image Parameters (for refinement) ---
    ...
    % Ensure tools are on the path (for local testing/compilation context)
    if ~isdeployed
    ...
    %% 4. Contrast Image (for refinement)
    ...
    % Linking tracklets to close the gaps
    ...
    % Delete track spots with non-positive gaps
